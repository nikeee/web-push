...
describe("Test Vapid Helpers", () => {
    ...
    const badInputs = [
      ...
      () => {
        ...
        vapidHelper.getVapidHeaders(
          VALID_AUDIENCE,
          VALID_SUBJECT_MAILTO,
          VALID_UNSAFE_BASE64_PUBLIC_KEY,
          VALID_PRIVATE_KEY,
  ...
  const validInputs = [
    ...
    contentEncoding => {
      ...
      return vapidHelper.getVapidHeaders(
        VALID_AUDIENCE,
        VALID_SUBJECT_LOCALHOST_MAILTO,
        VALID_PUBLIC_KEY,
        VALID_PRIVATE_KEY,
        contentEncoding,
      );
  ...
  function testValidInputs(contentEncoding) {
    validInputs.forEach((validInput, index) => {
      try {
        ...
        console.warn("Valid input call for getVapidHeaders() threw an " + "error. [" + index + "]");
        throw err;
      }
    });
  }
